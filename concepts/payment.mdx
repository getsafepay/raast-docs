...
| `P_CANCELLED` | The RTP request was withdrawn before authorization (for example, a merchant-initiated cancel or an expired request). Reaching this status emits the `payment.voided` webhook event; there is no separate `P_VOIDED` status. |
